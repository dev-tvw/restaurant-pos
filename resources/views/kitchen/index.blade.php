                            ...
                            <table class="table align-middle table-nowrap" id="sortTable">
                                ...
                                <tbody class="list form-check-all">
                                    ...
                                    <tr>
                                        ...
                                        <td class="name"><a href="#">{{$order->order_code}}</a></td>
                                        <td class="code">{{$order->customer->name}}</td>
                                        <td class="created_by">{{$order->item_count}}</td>
                                        <td class="updated_by">{{$order->grand_total}}</td>
                                        <td class="created_at"><span class="badge rounded-pill {{$order->status == 1 ? 'bg-success' : ($order->status == 2 ? 'bg-warning' : 'bg-danger')}} text-uppercase">{{$order->status == 1 ? 'Active' : ($order->status == 2 ? 'Completed' : 'Cancelled')}}</span></td>
                                        ...
                                        <td>
                                            @if($order->status == 0)
                                            <div class="edit">
                                                <a class="btn btn-sm btn-primary edit-item-btn" target="_blank" href="#"><i class="fa fa-print" aria-hidden="true"></i></a>
                                            </div>
                                            @else
                                            -
                                            @endif
